...
void ShiftBar_Tick(void)
{
	...
	float RPM_Idle = (float) Data_Car.RPM_Idle;
	float RPM_Low = (float) Data_Game.HS.RPM;
	float RPM_Downshift = (float) Data_Game.HS.RPM;
	if (Data_Game.HS.Gear > 0)
        ...
        if (Data_Game.HS.Gear > 1)
        {
            RPM_Downshift = Data_Car.RPM_Max *
                    (Data_Car.GearRatio[Data_Game.HS.Gear] / Data_Car.GearRatio[Data_Game.HS.Gear-1]) * (0.9 + 0.0015 * Data_Game.HS.Brake);
        }
        else
        {
            RPM_Downshift = -1;
	...
	UI08_t RPM_Percentage = (UI08_t) (RevsPercentage * (RPM_Now - RPM_Low) / (RPM_High - RPM_Low) / 10);
        ...
        float DownShiftPercentage = (1.0f*RPM_Now - 1.0f*RPM_Idle*2) / (1.0f*RPM_Downshift - 1.0f*RPM_Idle*2);

        if (Data_Game.HS.PitLimiter)
        {
            Blinker = Blinker%250;
            for(i = 0; i < 16; i++)
            {
                if (Blinker > 125 && (i % 2) == 0)
                    ShiftBar_Set(i, 0, 0, 0);
                else if (Blinker < 125 && (i % 2) == 0)
                    ShiftBar_Set(i, 10, 0, 7);

                if (Blinker > 125 && (i % 2) == 1)
                    ShiftBar_Set(i, 10, 0, 7);
                else if (Blinker < 125 && (i % 2) == 1)
                    ShiftBar_Set(i, 0, 0, 0);
            }
        }
        else
        {
            ...
            if (RPM_Now <= RPM_Downshift)
            {
                for(i = 5; i < 11; i++)
                {
                    ShiftBar_Set(i, 0, ((UI08_t)(15.0-15.0*DownShiftPercentage)), 0);
                }
            }
